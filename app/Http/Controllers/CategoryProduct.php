...
class CategoryProduct extends Controller
{
    ...
    public function save_category_product(Request $request){
        ...
        Session::put('message','Thêm danh mục thể loại thành công');
    ...
    public function show_category_home($id){
        ...
        $categoryById = DB::table('tbl_product')->join('tbl_category_product', 'tbl_product.category_id','=','tbl_category_product.category_id')->where('tbl_category_product.category_id',$id)->get();

        $category_name = DB::table('tbl_category_product')->where('tbl_category_product.category_id', $id)->limit(1)->get();

        return view('pages.category.show_category')->with('cate_product', $cate_product)->with('author', $author)->with('categoryById', $categoryById)->with('category_name', $category_name)->with('banchay_product', $SpBanChayNhat);
    }
